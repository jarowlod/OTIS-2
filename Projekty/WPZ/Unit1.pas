...
begin
  Memo1.Clear;
  Memo1.Append(WPZ.ulamek);
  Memo1.Append(DateToStr(WPZ.data_osadzenia));
  Memo1.Append('wpz: '+ DateToStr(WPZ.WPZ));
  if WPZ.WPZ = NullDate then Memo1.Append('NullDate');
  Memo1.Append(WPZ.orzeczenia[0].art);
  Memo1.Append(WPZ.orzeczenia[1].art);
end;
  ...
  for i:=0 to Count-1 do
    ...
    begin
      ...
      begin
        ranga:= 4;
        Result:= 'po 25';
        fKomunikaty.Append('Wykryto karę dożywocia w wierszu '+IntToStr(i+1));
      end else
      if (orzeczenia[i].lat=25)and(ranga<3) then
      begin
        ranga:= 3;
        Result:= 'po 15';
        fKomunikaty.Append('Wykryto karę 25 lat w wierszu '+IntToStr(i+1));
      end else
      if (orzeczenia[i].art.IndexOf(' 64§1')>=0)and(ranga<1) then
      begin
        ranga:=1;
        Result:='2/3';
        fKomunikaty.Append('Wykryto art.64§1 w wierszu '+IntToStr(i+1));
      end else
      if (orzeczenia[i].art.IndexOf(' 64§2')>=0)and(ranga<2) then
      begin
        ranga:=2;
        Result:='3/4';
        fKomunikaty.Append('Wykryto art.64§2 w wierszu '+IntToStr(i+1));
      end else
      if (orzeczenia[i].art.IndexOf(' 65§1')>=0)and(ranga<2) then
      begin
        ranga:=2;
        Result:='3/4';
        fKomunikaty.Append('Wykryto art.65§1 w wierszu '+IntToStr(i+1));
      end;
    end;
end;
...
begin
  ...
  begin
    ...
    begin
      WPZ:= wpz_po15;
      fKomunikaty.Append('WPZ ograniczone; max po 15 latach.');
    end;
  end;
  ...
  begin
    ...
    fKomunikaty.Append('WPZ z dniem osadzenia.');
  end;
end;
...
begin
  ...
  begin
    ...
    fKomunikaty.Append('Przepustki z dniem osadzenia.');
  end;
end;
...
begin
  ...
  begin
    ...
    fKomunikaty.Append('Postpenit z dniem osadzenia.');
  end;
end;
...
begin
  Result:= true;
  if data_osadzenia=NullDate then begin fKomunikaty.Append('Brak daty osadzenia.'); Result:= False; end;
  if Count   = 0 then begin fKomunikaty.Append('Brak wprowadzonych orzeczeń.'); Result:= False; end;
...
begin
  ...
  begin
    fKomunikaty.Append('Obostrzenie: po '+IntTostr(OBS_po_ilu)+' dniach. Uwzględnia okresy zaliczenia kary.');
...
begin
  ...
  begin
    ...
    fKomunikaty.Append('Brak uprawnien do WPZ. Kary zastępcze.');
    fisObliczono:= true;
    Result:= true;
  end;
end;
...
begin
  ...
  if IndexWPZ= -1 then begin fKomunikaty.Append('Brak kary zasadniczej.'); exit; end;
  ...
  if orzeczenia[IndexWPZ].koniec_kary = NullDate then begin fKomunikaty.Append('Brak daty końca kary.'); exit; end;

  fKomunikaty.Append('Obliczono po: '+ulamek);
...
var owz_wpz: TDate;
    ...
    dni_do_wpz: integer;
begin
  fKomunikaty.Append('Zastosowano owz.');
  ...
  begin
    ...
    begin
      fKomunikaty.Append('Zastosowano art. 81. Bez uprawnień do WPZ. Kara krótsza niż rok.');
      wpz:= NullDate;
      przepustki:= NullDate;
    ...
    fKomunikaty.Append('Zastosowano art. 81. Po roku od osadzenia, uwazględniono przesunięcie o kary zastępcze.');
    fisObliczono:= true;
  end;
end;
...
begin
  ...
  fKomunikaty.Append('Obliczono wpz dla dożywocia.');
  fisObliczono:= true;
  Result:= true;
end;
...
begin
  ...
  if ObliczKaryZastepcze then exit;
  if ObliczOBS then exit;
